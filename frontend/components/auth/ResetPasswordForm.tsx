...
export function ResetPasswordForm({ token }: { token?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (newPassword.length < 8 || !passwordPattern.test(newPassword)) { setError("Use at least 8 characters with uppercase, lowercase and a number."); return; }
    setError(null);
  ...
  return <form className="auth-form" onSubmit={submit} noValidate>
    ...
    <label>Confirm password<input name="confirmPassword" type={showPasswords ? "text" : "password"} autoComplete="new-password" minLength={8} required /></label>
    <label className="checkbox"><input type="checkbox" checked={showPasswords} onChange={(event) => setShowPasswords(event.target.checked)} />Show passwords</label>
