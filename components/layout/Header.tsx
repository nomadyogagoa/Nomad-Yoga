...
export function Header(){
  ...
  return <header className={`site-header${scrolled?" is-scrolled":""}`}><div className="nav-shell"><Link className="brand" href="/"><span className="brand-mark">✦</span><span>{siteConfig.name}</span></Link><nav className="desktop-nav">{visible.map(([l,h])=><Link key={l} href={h}>{l}</Link>)}</nav><div className="nav-actions">{featureFlags.memberLogin&&<Link className="text-button" href="/login">Login</Link>}<Link className="button button-small" href="/register">Join Us</Link></div><button className={`menu-button${open?" is-open":""}`} onClick={()=>setOpen(!open)} aria-label="Toggle menu" aria-expanded={open}><span/><span/></button></div>{open&&<nav className="mobile-nav">{visible.map(([l,h])=><Link key={l} href={h} onClick={()=>setOpen(false)}>{l}</Link>)}<Link href="/login" onClick={()=>setOpen(false)}>Member Login</Link><Link className="button" href="/register" onClick={()=>setOpen(false)}>Join Us</Link></nav>}</header>;
}
